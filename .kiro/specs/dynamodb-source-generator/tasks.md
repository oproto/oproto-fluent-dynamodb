...
### Problem: Multiple Code Generation Implementations Creating Confusion

**Current State Analysis:**
The codebase has THREE different code generation implementations that are causing confusion and test failures:

1. **`MapperGenerator.cs`** (Original, ~1089 lines)
   - Located in `Oproto.FluentDynamoDb.SourceGenerator/Generators/`
   - Calls `OptimizedCodeGenerator.GenerateOptimizedToDynamoDbMethod()` for ToDynamoDb
   - Calls `OptimizedCodeGenerator.GenerateOptimizedFromDynamoDbMethod()` for FromDynamoDb
   - Then adds its own `ToDynamoDbMultiple` method
   - Generates complete entity implementation with all interface methods

2. **`OptimizedCodeGenerator.cs`** (Performance layer, ~601 lines)
   - Located in `Oproto.FluentDynamoDb.SourceGenerator/Performance/`
   - Generates "optimized" versions of ToDynamoDb and FromDynamoDb
   - Uses `[MethodImpl(MethodImplOptions.AggressiveInlining)]`
   - Pre-allocates dictionaries with exact capacity
   - Does NOT generate the class declaration or `: IDynamoDbEntity` interface implementation
   - Only generates method bodies

3. **`AdvancedPerformanceOptimizations.cs`** (Advanced layer, ~1039 lines)
   - Located in `Oproto.FluentDynamoDb.SourceGenerator/Performance/`
   - Generates "ultra-optimized" versions with ArrayPool, span operations
   - Includes complete class structure with `: IDynamoDbEntity`
   - Has its own `GenerateOptimizedEntityCode()` entry point
   - Uses parallel code generation for large entities
   - Appears to be a COMPLETE REPLACEMENT for MapperGenerator, not a helper

**The Confusion:**
- Tests reference `ToDynamoDbMultiple` which is generated by `MapperGenerator`
- But `MapperGenerator` delegates to `OptimizedCodeGenerator` for the main methods
- `AdvancedPerformanceOptimizations` seems to be an alternative complete implementation
- It's unclear which implementation is actually being used in production
- Tests are failing because they expect output from one implementation but get output from another

**Questions That Need Answers:**
1. Is `AdvancedPerformanceOptimizations` meant to REPLACE `MapperGenerator` entirely?
2. Or is it meant to be an OPTIONAL performance mode?
3. Why does `MapperGenerator` call `OptimizedCodeGenerator` instead of generating code directly?
4. Should there be ONE canonical implementation or multiple implementations for different scenarios?
5. Why do tests expect `ToDynamoDbMultiple` when the interface `IDynamoDbEntity` requires it?

**Impact on Tests:**
- Tests expect specific code patterns from `MapperGenerator`
- But actual generated code comes from `OptimizedCodeGenerator` or `AdvancedPerformanceOptimizations`
- Generated code is missing expected elements like:
  - `ToDynamoDbMultiple` method (expected by tests and interface)
...
- [x] 40. **CRITICAL: Consolidate code generation to single implementation**
  - **Problem Confirmed:** Three implementations exist due to incremental "optimization" tasks:
    1. `MapperGenerator.cs` (original) - delegates to OptimizedCodeGenerator
    2. `OptimizedCodeGenerator.cs` (added for performance) - only generates method bodies
    3. `AdvancedPerformanceOptimizations.cs` (added later) - complete implementation but **MISSING `ToDynamoDbMultiple`**
  - **Decision:** Keep ONLY `MapperGenerator.cs` as the single source of truth
  - **Implementation:**
    - **DELETE** `Oproto.FluentDynamoDb.SourceGenerator/Performance/OptimizedCodeGenerator.cs`
    - **DELETE** `Oproto.FluentDynamoDb.SourceGenerator/Performance/AdvancedPerformanceOptimizations.cs`
    - **UPDATE** `MapperGenerator.cs` to generate all code directly (no delegation)
    - **KEEP** performance optimizations (pre-allocated dictionaries, AggressiveInlining) in MapperGenerator
    - **ENSURE** `ToDynamoDbMultiple` is always generated (required by IDynamoDbEntity interface)
    - **ENSURE** complete class structure with `: IDynamoDbEntity` is generated
  - **Verification:**
    - All tests pass with single implementation
    - Generated code compiles without errors
    - Generated code implements complete IDynamoDbEntity interface
    - No references to deleted files remain
  - _Requirements: Architectural clarity, test reliability, maintainability_

- [x] 41. **Remove ToDynamoDbMultiple from interface and clean up multi-item concept**
  - **Problem:** `ToDynamoDbMultiple` is a leftover from the old multi-item design that was replaced in Task 23
  - **New design (from Task 23):** `ToListAsync<T>()` and `ToCompositeEntityAsync<T>()` handle the read side
  - **Current usage:**
    - Interface `IDynamoDbEntity` defines `ToDynamoDbMultiple` (REMOVE THIS)
    - `FluentResultsExtensions.GetDynamoDbItemsResult<T>()` calls it (REMOVE THIS - no production usage)
    - Tests expect it (UPDATE TESTS)
  - **Implementation:**
    - **REMOVE** `ToDynamoDbMultiple<TSelf>` from `IDynamoDbEntity` interface
    - **REMOVE** `GetDynamoDbItemsResult<T>()` from `FluentResultsExtensions` (no production usage found)
    - **REMOVE** `GetDynamoDbItemsResult` documentation from FluentResults README
    - **UPDATE** `MapperGenerator` to NOT generate `ToDynamoDbMultiple` method
    - **UPDATE** all tests to not expect `ToDynamoDbMultiple`
  - **Rationale:** 
    - **Write operations:** `ToDynamoDb()` returns a single item dictionary - that's all you need for PutItem
    - **Read operations:** `ToListAsync()` and `ToCompositeEntityAsync()` handle queries
    - **Batch writes:** If you need to write multiple entities, just call `ToDynamoDb()` on each one
    - **Composite entities:** The "multiple items per entity" concept is handled on the READ side by `ToCompositeEntityAsync()`
    - No valid use case for a method that returns `List<Dictionary<string, AttributeValue>>` from a single entity
  - _Requirements: API consistency, remove dead code, clear separation of concerns_

- [x] 42. **Ensure MapperGenerator generates complete, self-contained code**
  - **After deleting the other generators and removing ToDynamoDbMultiple, verify MapperGenerator generates:**
    - Complete file header with auto-generated comment
    - All necessary using statements
    - Namespace declaration
    - Class declaration with `public partial class {ClassName}` (NO `: IDynamoDbEntity` - it's a partial class!)
    - `ToDynamoDb<TSelf>` method with proper type conversions (single item only)
    - `FromDynamoDb<TSelf>` method (single item version)
    - `FromDynamoDb<TSelf>` method (multi-item version - overload for composite entities)
  ...
  - **Keep performance optimizations:**
    ...
    - `[MethodImpl(MethodImplOptions.AggressiveInlining)]` where appropriate
    - Efficient string operations
  - _Requirements: Complete code generation, interface compliance_

- [x] 43. **Update all generator tests to match MapperGenerator output**
  - **After consolidation and ToDynamoDbMultiple removal, update test expectations:**
    - Tests should expect code from `MapperGenerator` only
    - Remove expectations for `OptimizedCodeGenerator` or `AdvancedPerformanceOptimizations` patterns
    - **REMOVE** expectations for `ToDynamoDbMultiple` method (it's being deleted)
    - Verify tests check for complete class structure
    - Verify tests check for all required interface methods (excluding ToDynamoDbMultiple)
  - **Specific test fixes needed:**
    - `MapperGeneratorTests.GenerateEntityImplementation_WithBasicEntity_ProducesCorrectCode`
    - `MapperGeneratorTests.GenerateEntityImplementation_WithMultiItemEntity_GeneratesMultiItemMethods` (update for composite entity pattern)
    - `MapperGeneratorTests.GenerateEntityImplementation_WithCollectionProperties_GeneratesJsonSerialization`
    - `MapperGeneratorTests.GenerateEntityImplementation_WithDifferentPropertyTypes_GeneratesCorrectConversions`
    - Integration tests expecting specific generated code format
    - **REMOVE** any tests specifically for `ToDynamoDbMultiple` generation
  - **Note:** KeysGenerator and FieldsGenerator are separate and should remain unchanged
  - _Requirements: Test accuracy, test reliability_

- [x] 44. **Clean up and document the simplified architecture**
  - **Remove references to deleted files:**
    - Search for any imports/usings of `OptimizedCodeGenerator` or `AdvancedPerformanceOptimizations`
    - Remove any configuration or flags that switched between implementations
    - Update any documentation that referenced the deleted files
  - **Document the final architecture:**
    - `MapperGenerator` - generates entity mapping code (ToDynamoDb, FromDynamoDb, etc.)
    - `KeysGenerator` - generates key builder methods
    - `FieldsGenerator` - generates field name constants
    - `EntityAnalyzer` - analyzes entity classes and produces EntityModel
  - **Add inline documentation:**
    - Add XML comments to MapperGenerator explaining its purpose
    - Document performance optimizations used in generated code
    - Explain why we generate specific patterns (e.g., pre-allocated dictionaries)
  - _Requirements: Maintainability, developer onboarding, architectural clarity_


- [x] 45. **Fix generated code header format and structure expectations**
  - **Problem:** Tests expect specific header comments and code structure that MapperGenerator, KeysGenerator, and FieldsGenerator don't currently produce
  - **Affected tests (15 failures):**
    - All 7 KeysGenerator tests expecting specific format
    - 1 FieldsGenerator test (reserved words)
    - 7 integration/edge case tests expecting specific entity code format
  - **Root cause:** After consolidating to single MapperGenerator implementation, generated code format changed
  - **Implementation:**
    - **Update MapperGenerator** to include proper header:
      ```csharp
      // <auto-generated />
      // This code was generated by the DynamoDB Source Generator.
      // Changes to this file may be lost when the code is regenerated.
      ```
    - **Update KeysGenerator** to match expected format with proper headers
    - **Update FieldsGenerator** to match expected format with proper headers
    - **Ensure consistent formatting** across all generators (indentation, spacing, comments)
    - **Update all generator tests** to match the actual generated code structure
    - **Verify generated code compiles** and implements IDynamoDbEntity correctly
  - **Test fixes needed:**
    - `KeysGeneratorTests.GenerateKeysClass_WithNullableTypes_GeneratesNullChecks`
    - `KeysGeneratorTests.GenerateKeysClass_WithDateTimeType_GeneratesFormattedString`
    - `KeysGeneratorTests.GenerateKeysClass_WithPartitionAndSortKey_GeneratesAllKeyBuilders`
    - `KeysGeneratorTests.GenerateKeysClass_WithPartitionKeyOnly_GeneratesPartitionKeyBuilder`
    - `KeysGeneratorTests.GenerateKeysClass_WithGuidType_GeneratesToStringConversion`
    - `KeysGeneratorTests.GenerateKeysClass_WithCustomKeyFormat_ParsesFormatCorrectly`
    - `FieldsGeneratorTests.GenerateFieldsClass_WithReservedWords_HandlesCorrectly`
    - `EdgeCaseTests.SourceGenerator_WithVeryComplexRelationshipPatterns_HandlesCorrectly`
    - `EdgeCaseTests.SourceGenerator_WithNestedNamespaces_HandlesCorrectly`
    - `EdgeCaseTests.SourceGenerator_WithReservedKeywords_EscapesCorrectly`
    - `EdgeCaseTests.SourceGenerator_WithGenericTypeConstraints_HandlesCorrectly`
    - `EdgeCaseTests.SourceGenerator_WithCircularReferences_HandlesGracefully`
    - `EdgeCaseTests.SourceGenerator_WithEmptyEntity_GeneratesMinimalCode`
    - `SourceGeneratorPerformanceTests.SourceGenerator_WithManyRelatedEntities_HandlesEfficiently`
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithRelatedEntities_GeneratesRelationshipMapping`
  - _Requirements: 2.1, 2.2, 2.3, 3.1, 3.2, 14.5_

- [x] 46. **Fix diagnostic expectation mismatches and missing diagnostics**
  - **Problem:** Tests expect specific diagnostic IDs that don't match actual behavior after Task 39 removed DYNDB027
  - **Affected tests (4 failures):**
    - `EntityAnalyzerTests.AnalyzeEntity_WithAmbiguousRelatedEntityPattern_ReportsWarning` - expects DYNDB023 but test looks for different ID
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithPerformanceIssues_GeneratesActionableDiagnostics` - expects DYNDB027 (removed in Task 39)
    - `SourceGeneratorPerformanceTests.SourceGenerator_WithManyGSIs_HandlesEfficiently` - expects diagnostics but gets none
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithScalabilityIssues_GeneratesHelpfulWarnings` - expects DYNDB027 (removed in Task 39)
  - **Root cause:** Task 39 removed flawed DYNDB027 scalability detection, but tests still expect it
  - **Implementation:**
    - **Update test expectations** for tests that expected DYNDB027 to either:
      - Remove the test if it's no longer valid (scalability can't be detected at compile time)
      - Update to expect different diagnostics that ARE valid
    - **Fix EntityAnalyzerTests.AnalyzeEntity_WithAmbiguousRelatedEntityPattern_ReportsWarning**:
      - Test expects DYNDB023 but assertion looks for wrong diagnostic ID
      - Update assertion to match actual diagnostic ID being generated
    - **Fix SourceGeneratorPerformanceTests.SourceGenerator_WithManyGSIs_HandlesEfficiently**:
      - Test expects GSI overuse warning (>5 GSIs) but entity only has 3 GSIs
      - Either update test entity to have >5 GSIs or update test expectation
    - **Document diagnostic behavior** in tests to prevent future confusion
  - **Test fixes needed:**
    - `EntityAnalyzerTests.AnalyzeEntity_WithAmbiguousRelatedEntityPattern_ReportsWarning` - fix assertion
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithPerformanceIssues_GeneratesActionableDiagnostics` - remove DYNDB027 expectation
    - `SourceGeneratorPerformanceTests.SourceGenerator_WithManyGSIs_HandlesEfficiently` - add more GSIs or update expectation
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithScalabilityIssues_GeneratesHelpfulWarnings` - remove or update test
  - _Requirements: 14.1, 14.2, 14.3, 14.4_

- [x] 47. **Update test expectations to accept `: IDynamoDbEntity` in generated partial class declarations**
  - **Problem:** Tests expect generated code WITHOUT `: IDynamoDbEntity`, but the generator correctly includes it for better user experience
  - **Affected tests (5 failures):**
    - `MapperGeneratorTests.GenerateEntityImplementation_WithBasicEntity_ProducesCorrectCode` - expects code WITHOUT `: IDynamoDbEntity`
    - `EdgeCaseTests.SourceGenerator_WithVeryComplexRelationshipPatterns_HandlesCorrectly` - expects code WITHOUT `: IDynamoDbEntity`
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithComplexTypes_GeneratesCorrectConversions` - expects code WITHOUT `: IDynamoDbEntity`
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithRelatedEntities_GeneratesRelationshipMapping` - expects code WITHOUT `: IDynamoDbEntity`
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithCompleteEntity_GeneratesAllExpectedFiles` - expects code WITHOUT `: IDynamoDbEntity`
  - **Root cause:** Task 42 specified "NO `: IDynamoDbEntity`" but this decision should be reversed for better UX
  - **Why including `: IDynamoDbEntity` in generated code is CORRECT:**
    - **Better user experience**: User only needs to add `[DynamoDbTable]` attribute - one step instead of two
    - **Less error-prone**: User can't forget to add the interface to their partial class declaration
    - **Self-documenting**: Generated code explicitly shows it implements IDynamoDbEntity
    - **Valid C#**: Redeclaring interfaces in partial classes is perfectly legal and doesn't cause warnings
    - **Common pattern**: Follows the pattern of other successful C# source generators (e.g., System.Text.Json)
    - **Clearer intent**: Makes it obvious that the generated code is providing the interface implementation
  - **User experience comparison:**
    - **WITHOUT interface in generated code** (old approach):
      ```csharp
      [DynamoDbTable("users")]
      public partial class User : IDynamoDbEntity  // User must remember to add this
      {
          [PartitionKey] public string Id { get; set; }
      }
      ```
    - **WITH interface in generated code** (current/correct approach):
      ```csharp
      [DynamoDbTable("users")]
      public partial class User  // User just adds attribute - simpler!
      {
          [PartitionKey] public string Id { get; set; }
      }
      // Generated code includes: public partial class User : IDynamoDbEntity
      ```
  - **Implementation:**
    - **Update test expectations** to EXPECT `: IDynamoDbEntity` in generated code:
      - Change assertions from `.NotContain("public partial class {ClassName} : IDynamoDbEntity")`
      - To: `.Contain("public partial class {ClassName} : IDynamoDbEntity")`
    - **Update test documentation** to explain why the interface is included
    - **Verify MapperGenerator** currently generates the interface correctly (it does)
  - **Test fixes needed:**
    - `MapperGeneratorTests.GenerateEntityImplementation_WithBasicEntity_ProducesCorrectCode` - update to expect `: IDynamoDbEntity`
    - `EdgeCaseTests.SourceGenerator_WithVeryComplexRelationshipPatterns_HandlesCorrectly` - update to expect `: IDynamoDbEntity`
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithComplexTypes_GeneratesCorrectConversions` - update to expect `: IDynamoDbEntity`
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithRelatedEntities_GeneratesRelationshipMapping` - update to expect `: IDynamoDbEntity`
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithCompleteEntity_GeneratesAllExpectedFiles` - update to expect `: IDynamoDbEntity`
  - **Note:** This reverses the decision made in Task 42 based on better understanding of UX and C# source generator best practices
  - _Requirements: 2.1, 2.2, 2.3, 3.1, 3.2, Developer Experience_

- [x] 48. **Fix remaining test failures from related entity mapping and key builder expectations**
  - **Problem:** Three tests are failing due to missing or incorrect expectations about generated code structure
  - **Affected tests (3 failures):**
    - `EdgeCaseTests.SourceGenerator_WithVeryComplexRelationshipPatterns_HandlesCorrectly` - expects "// Populate related entity properties based on sort key patterns" comment in generated code
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithRelatedEntities_GeneratesRelationshipMapping` - expects "// Populate related entity properties based on sort key patterns" comment in generated code
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithCompleteEntity_GeneratesAllExpectedFiles` - expects `return "tenant#" + tenantId;` format in keys code
  - **Root cause:** Tests expect code patterns that may have been removed or changed in previous generator consolidation tasks
  - **Analysis needed:**
    - **For related entity mapping tests:** Check if MapperGenerator currently generates the "// Populate related entity properties based on sort key patterns" comment for entities with relationships
      - If YES: Tests are correct, no changes needed (generator already works)
      - If NO: Determine if this is expected behavior after consolidation or if generator needs to be updated
    - **For key builder test:** Check if KeysGenerator generates the expected format `var keyValue = "tenant#" + tenantId; return keyValue;` or a different format
      - The test output shows the generator DOES produce this code, so the test assertion may be looking in the wrong place or using wrong string matching
  - **Implementation options:**
    - **Option A (Update tests):** If the generator behavior is correct but tests are checking incorrectly:
      - Update test assertions to match actual generated code structure
      - Verify tests are reading the correct generated file
      - Check for whitespace or formatting differences in string matching
    - **Option B (Update generator):** If the generator should produce these patterns but doesn't:
      - Update MapperGenerator to include related entity mapping comments for multi-item entities
      - Ensure KeysGenerator produces the expected key builder format
    - **Option C (Remove expectations):** If these patterns are no longer relevant after consolidation:
      - Remove or update test expectations to match new simplified generator output
      - Document why these patterns were removed
  - **Investigation steps:**
    1. Run each failing test individually and examine the actual generated code output
    2. Compare expected vs actual generated code to identify exact differences
    3. Review MapperGenerator and KeysGenerator source to understand current behavior
    4. Determine if tests or generators need updating based on design decisions
  - **Test fixes needed:**
    - `EdgeCaseTests.SourceGenerator_WithVeryComplexRelationshipPatterns_HandlesCorrectly` - investigate and fix
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithRelatedEntities_GeneratesRelationshipMapping` - investigate and fix
    - `EndToEndSourceGeneratorTests.SourceGenerator_WithCompleteEntity_GeneratesAllExpectedFiles` - investigate and fix
  - **Note:** This task requires investigation before implementation to determine root cause
  - _Requirements: 2.1, 2.2, 2.3, 3.1, 3.2, Related entity mapping, Key builder generation_
